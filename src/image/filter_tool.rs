...
impl Image {
    pub fn pixelate(&mut self, top_x: i32, top_y: i32, p_width: u32, p_height: u32, block_size: u32, blur_type: &str) {
        ...
        // x and y represent marked region by pixelate
        let mut x;
        let mut y;
        // saving each block's pixel in block
        ...
        for row in (0..p_height).step_by(block_size as usize) {
            for col in (0..p_width).step_by(block_size as usize) {
                x = top_x as u32 + col;
                y = top_y as u32 + row;
                // for saving all x & y coordinate in block 
    ...
    pub fn gaussian_blur(&mut self, sigma: f64, top_x: i32, top_y: i32, width: u32, height: u32, is_standalone: bool) {
        ...
        for idx in 0..num_pass {
            ...
            self.box_blur_v(&src, &mut tgt, width, height, box_size[idx as usize] / 2);
        }
    ...
    fn box_blur_h(&mut self, src: &[u8], tgt: &mut [u8], width: u32, height:u32, radius: u32) {
        ...
        let mut box_sum = |row, col, start: i32, end: i32| {
            for idx in 0..(radius * 2 + 1) {
                let col = cmp::min(cmp::max(start + idx as i32, 0), width as i32 -1)as u32;
                let idx2 = (row * width) as usize + col as usize;
                for c in 0..channel_count {
                    running_sum[c] += src[idx2 * channel_count + c] as u32;
                }
    ...
    fn box_for_gaussian(&self, sigma: f64, n: u32) -> Vec<u32> {
        ...
        let m_ideal = (12.0 * sigma * sigma -n as f64 * wl * wl -4.0 * n as f64 * wl -3.0 * n as f64) / (-4.0 * wl -4.0);
        let m = m_ideal.round();
    ...
    // For cartoonifyng a image
    /*
    pub fn cartoonify(&mut self, radius: u32, sigma_r: f64, iter_count: u32, incr: bool) {
        self.bilaternal_filter(radius, sigma_r, iter_count, incr);
    ...
    */
    ...
    fn median_filter(&mut self, radius: u32) {
        ...
        if radius  > self.height / 2 || radius > self.width / 2 {
            log! ("take radius larger than half of width and height of image");
            return
        }
        ...
        let mut update_hist_h = |row: i32, col: i32, histogram: &mut Vec<Vec<u32>>, pixels: &[u8]| {
            ...
            for i in 0..kernel_width {
                ...
                let col_to_add = (row + radius + 1).min(img_width as i32 -1) as u32;
    ...
    pub fn bilaternal_filter(&mut self, radius: u32, sigma_r: f64, iter_count: u32, incr: bool) {
        //  add validation
