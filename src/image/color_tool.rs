...
impl Image {
    pub fn rgb_to_hsi(&mut self) {
        ...
        for idx in 0..size {
            ...
            hue = (0.5 * ((r-g)) / (((r - g) * (r - g) + (r - b) * (g -b)).sqrt() + 0.001)).acos();
            if b > g {
                hue = two_pi - hue
            }
            // for hue
    ...
    pub fn manual_adjust_intensity(&mut self, gain: f64, bins: f64) {
        let new_intensity = |v: &f64| -> f64 {
            let new_v = *v * gain + bins;
    ...
    pub fn auto_adjust_intensity(&mut self) {
        ...
        for idx in 1..256 { // It's same as fibonacci series
