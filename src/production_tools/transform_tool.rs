...
impl Image {
  pub fn crop(&mut self, top_x: i32, top_y: i32, width: u32, height: u32) {
    ...
    let mut current_idk: usize;
    for row in 0..height {
      for col in 0..width {
        old_x = top_x as u32 + col; // x & y are positions of original pixel vectors
        old_y = top_y as u32 + row;
        old_idx = (old_y * self.width + old_x) as usize;
        current_idk = (row * width + col) as usize;
        new_pixels[current_idk * 4 + 0] = self.pixels[old_idx * 4 + 0];
        new_pixels[current_idk * 4 + 1] = self.pixels[old_idx * 4 + 1];
        new_pixels[current_idk * 4 + 2] = self.pixels[old_idx * 4 + 2];
        new_pixels[current_idk * 4 + 3] = self.pixels[old_idx * 4 + 3];
      }
    }
  ...
  pub fn rotate(&mut self, clockwise: bool) {
    ...
    for row in 0..h {
      for col in 0..w {
        ...
        new_idx = new_y * h + new_x; // new hight will we original width
        current_idk = row * w  + col;
        
        new_pixels[new_idx * 4 + 0] = self.pixels[current_idk * 4 + 0];
        new_pixels[new_idx * 4 + 1] = self.pixels[current_idk * 4 + 1];
        new_pixels[new_idx * 4 + 2] = self.pixels[current_idk * 4 + 2];
        new_pixels[new_idx * 4 + 3] = self.pixels[current_idk * 4 + 3];
      }
    }
    self.pixels = new_pixels;
    self.width = h as u32;
  ...
  // To Rotate image by a specifiesd angle 
  pub fn rotate_by(&mut self) {
    // We wil write it later.
    self.last_operation = Operation::Transform
  }
  ...
  pub fn flip_h(&mut self) {
    ...
    let mut tmp_row = vec![0_32; w];
    let tmp_row_ref = tmp_row.as_mut_slice();

    for row in self.pixels.chunks_mut(w * 4) {
      for pixel_idx in 0..w {
        tmp_row_ref[pixel_idx] =
            ((row[pixel_idx * 4 + 0] as u32) << 24) | ((row[pixel_idx * 4 + 1] as u32) << 16) |
            ((row[pixel_idx * 4 + 0] as u32) << 8) | ((row[pixel_idx * 4 + 3] as u32) << 0);
      }
      tmp_row_ref.reverse();
  ...
  // To Scale the image(change aspect ratio)
  ...
  // This is an independent fn for deciding bilinear interpolation
  fn bilinear_interpolate(&self, x: f64, y: f64, pixel_buf: &mut Vec<u8>){
    let (mut tl, mut tr, mut bl, mut br) = ((x.floor() as u32, y.floor() as u32),
