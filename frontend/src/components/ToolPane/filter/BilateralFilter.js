...
export default class BilateralFilter extends Component {
  ...
  bf = () => {
    setTimeout(() => {
      ...
      this.wasm_img.bilateral_filter(
        this.state.sigma_domain,
        this.state.sigma_range,
        iter_count,
        incr
      );
      this.props.redraw();
    ...
    return (
      <div style={{ marginBottom: '180x', color: '#CCC' }}>
        ...
        <p style={{ fontSize: '12px', marginTop: '18px', color: '#ddd' }}>
          when the above 2 sliders approach max value(takes long time to
          finish), this filter will generate cartoonish feel.
        </p>
