...
class AccordionMenu extends Component<AccordionMenuProps> {
  redraw = (_reposition?: boolean) => {
    ...
    createImageBitmap(new ImageData(new Uint8ClampedArray(pixels), w, h)).then(
      (img) => {
        imgObj.imgBuff = img;
        const ratio = this.props.zoomRatio;
  ...
  render() {
    ...
    return (
      <AccordionHeader close={this.props.close} label={label}>
        {tool}
      </AccordionHeader>
    );
  }
      ...
      <button id="editor-goback-btn" onClick={close}>
        <svg viewBox="0 0 20 20" width="100%" height="100%" fill="#ccc" pointerEvents="none">
          <g fillRule="evenodd">
            <path
              fillRule="nonzero"
              d="M10 19a9 9 0 1 0 0-18 9 9 0 0 0 0 18zm0 1C4.477 20 0 15.523 0 10S4.477 0 10 0s10 4.477 10 10-4.477 10-10 10z"
            />
