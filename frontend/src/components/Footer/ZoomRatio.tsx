...
interface ZoomRatioOwnProps {
  resizeCanvas: EditorCallbacks['resizeCanvas'];
}

type ZoomRatioProps = ConnectedProps<typeof connector> & ZoomRatioOwnProps;

class ZoomRatio extends Component<ZoomRatioProps> {
  onResize = (evt: React.ChangeEvent<HTMLInputElement>) =>
    this.props.setZoomRatio(parseFloat(evt.target.value));

  setRatio = (evt: React.MouseEvent<HTMLButtonElement>) => {
    const zoomRatio = this.props.zoomRatio;
    if (zoomRatio > 4 || zoomRatio < 0.2) {
      return;
    }

    const zoomBtnCls = evt.currentTarget.classList;
      ...
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <button className="resize-view-btn fit-to-screen" onClick={this.setRatio}>
          <svg viewBox="-4 -4 22 22" width="22" height="22" fill="white" pointerEvents="none">
            <path
              fillRule="evenodd"
              d="M11.2 4H6V3h7v7h-1V4.6l-8.3 8.3-.7-.7L11.2 4zM0 0h16v16H0V0zm1 1v14h14V1H1z"
            />
          </svg>
        </button>
...
export default connector(ZoomRatio);
